...
const submitTriageReport = async (req, res) => {
  try {
    ...
    try {
      ...
      if (response.data && response.data.output) {
        ...
        if (questionMatch && questionMatch[1]) {
            ...
            // Only add if not "None" and has actual length
        ...
        aiAnalysis = { 
            ...
            reasoning: reasoning,
            follow_up_questions: questions, // <--- Frontend needs this Array!
    ...
    // We save the JSON object of the analysis so the dashboard can read it later if needed
    const values = [
      enrichedSymptoms,
      triageCategory,     
      location,
